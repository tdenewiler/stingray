...
long binaryToCalibratedAnalogVoltage_hw130( u3CalibrationInfo *caliInfo, uint8 positiveChannel, uint8 negChannel, uint16 bytesVoltage, double *analogVoltage )
{
	...
	else if ( negChannel == 32 ) {
		if ( caliInfo->highVoltage == 1 && positiveChannel >= 0 && positiveChannel < 4 ) {
			*analogVoltage = ( ( ( ( double )bytesVoltage - 16464.0 ) * 0.000637 ) + 0.019 );
			//*analogVoltage =
			//	((bytesVoltage / 65536.0) * caliInfo->vref * 2.0) - caliInfo->vref;
			//*analogVoltage = ((caliInfo->ainDiffSlope * (double)bytesVoltage)
			//	+ caliInfo->ainDiffOffset + caliInfo->vref);
//		* ((caliInfo->hvAINSlope[positiveChannel] / caliInfo->ainSESlope)
//		+ caliInfo->hvAINOffset[positiveChannel]);
			/*
					printf("---------------------------------\n");
					printf("bytesVoltage: %u\n", bytesVoltage);
					printf("ainDiffSlope: %f\n", caliInfo->ainDiffSlope);
					printf("ainDiffOffset: %f\n", caliInfo->ainDiffOffset);
					printf("vref: %f\n", caliInfo->vref);
					printf("vreg: %f\n", caliInfo->vreg);
					printf("hvAINSlope %d: %f\n", positiveChannel, caliInfo->hvAINSlope[positiveChannel]);
					printf("ainSESlope: %f\n", caliInfo->ainSESlope);
					printf("ainSEOffset: %f\n", caliInfo->ainSEOffset);
					printf("hvAINOffset %d: %f\n", positiveChannel, caliInfo->hvAINOffset[positiveChannel]);
					*/
		}
		else {
			*analogVoltage = caliInfo->ainDiffSlope * ( ( double )bytesVoltage )
...
long eAIN( HANDLE Handle, u3CalibrationInfo *CalibrationInfo, long ConfigIO, long *DAC1Enable, long ChannelP, long ChannelN, double *Voltage, long Range, long Resolution, long Settling, long Binary, long Reserved1, long Reserved2 )
{
	...
	if ( hwver >= 1.30 && hv == 1 && ChannelP < 4 ) {} else if ( ConfigIO != 0 ) {
